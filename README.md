...
| **`/api/proposals`** | A **third** separate endpoint carrying `netframe-proposal-dashboard-feed/v1` - which incident proposals Jarvis has prepared and is waiting on an owner for. Awareness only: the wall has no accept, dismiss, publish or select path, and a count is shown only when the intake collector could actually see. Point a consumer at another host's feed with `NFM_PROPOSAL_FEED_URL`; leave it unset to read the local projection (`NFM_PROPOSAL_FEED_FILE`). Neither is a secret. |
